...
def run_one(
    ...
    try:
        for turn in selected_turns:
            ...
            data, latency_s = run_request(endpoint, model, messages, osl, extra_inputs, timeout, api_key)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--api-key", default=None, help="API key for Authorization: Bearer header")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as executor:
        futures = [
            executor.submit(run_one, sample, endpoint, args.model, args.osl, extra_inputs, args.timeout, args.api_key)
